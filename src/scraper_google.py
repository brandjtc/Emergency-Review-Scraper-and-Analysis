...
existing_app_names = review_collection.distinct('app_name')
...
def scrape_reviews(app_info):
    ...
    if app_name in existing_app_names:
        print(f"App {app_name} already exists in the database. Skipping...")
        return
    ...
    if app_info_collection.find_one({"appId": app_id}):
        print(f"App {app_name} already exists in app_info_collection. Skipping...")
        return
    else:
