...
RESET=False
MINLENGTH=250
FILEPATH="./Generated Files/Storage_BR/"
SLEEPTIME=0
...
def aiInputHelperFunc(app_name):
	...
	review=review_collection.find({"app_name":app_name,"$expr": {"$gte": [{"$strLenBytes": "$content"}, MINLENGTH]}},{"_id":1,"content":1,"translated_content":1,"language":1})
	...
	for x in range(reviewStartVar,maxVal):
		if (review[x]['content']!=None) and (len(review[x]['content'])>=MINLENGTH):
